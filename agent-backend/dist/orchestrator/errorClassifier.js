...
exports.buildTargetedFixPrompt = buildTargetedFixPrompt;
...
function extractTsFiles(errorText) {
    const files = new Set();
    let m;
...
function extractBuildFiles(errorText) {
    const files = new Set();
 ...
 */
function classifyError(errorLogs) {
    ...
    if (errorLogs.includes('error TS') || errorLogs.startsWith('TypeScript TypeCheck failed')) {
        ...
        (0, logger_1.log)('XML', 'classified', { type: result.type, files: result.affectedFiles, summary: result.summary });
        return result;
    }
    // Vite build failures
    ...
    (0, logger_1.log)('XML', 'classified', { type: result.type, summary: result.summary });
    return result;
}
/**
 ...
 */
function buildTargetedFixPrompt(classified, fileContents) {

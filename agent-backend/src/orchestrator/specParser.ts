...
export interface ComponentSpec {
    filename: string;          // e.g. 'src/components/sections/Hero.tsx'
 ...
 */
export function extractSharedContext(specContent: string): string {
 ...
 */
export function parseSpecComponents(specContent: string): ComponentSpec[] {
    ...
    const filenames = rawFilenames.map(line => {
        ...
        if (name === 'App.tsx' || name === 'page.tsx') return null;
        // Already has a path prefix (e.g. components/GlassCard.tsx)
        if (name.includes('/')) return `src/${name}`;
 ...
 */
export function getComponentTier(componentName: string): 1 | 2 {
